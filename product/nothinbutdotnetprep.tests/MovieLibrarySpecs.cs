...
namespace nothinbutdotnetprep.tests
{
    public class MovieLibrarySpecs
    {
        ...
        public class when_accessing_the_list : concern
        {
            context c = () =>
            {
            ...
            it should_get_all_of_the_movies = () =>
            {
        ...
        public class when_searching_for_movies : searching_and_sorting_concerns_for_movie_library
        {
            ...
            {
                var results = sut.all_movies().all_matching(
            ...
            {
                var results = sut.all_movies().all_matching(
                    Where<Movie>.does_not_have_a(movie => movie.production_studio).equal_to(ProductionStudio.Pixar)) ;
            ...
            {
                var results = sut.all_movies().all_matching(Where<Movie>.has_an(movie => movie.date_published.Year)
                    .between(1982,2003));
            ...
            {
                var results = sut.all_movies().all_matching(Where<Movie>.has_a(movie => movie.genre).equal_to(Genre.kids));
                results.should_only_contain(a_bugs_life, shrek, cars);
            };

            it should_be_able_to_find_all_action_movies = () =>
            {
                var results = sut.all_movies().all_matching(movie => movie.genre == Genre.action);
